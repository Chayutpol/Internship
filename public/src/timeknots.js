var TimeKnots = {
  ...
  draw: function(id, events, options){
    ...
    .style("width", "300px")
    var svg = d3.select(id).append('svg').attr("width", cfg.width).attr("height", cfg.height).attr("id", "svg-time");
    ...
    .attr("cx", function(d){
      ...
      if(cfg.dateDimension){
        ...
        if(d.value < 0)
          var dateValue = d.name +" <small>("+"~BC"+")</small>";
        else
          var dateValue = d.name +" <small>("+d.value+" AD)</small>";
      }
      d3.select(this)
    ...
    svg.on("mousemove", function(){
        var elem = document.getElementById(id.substring(1, id.length));
        tipPixelsX = elem.getBoundingClientRect().left+window.pageXOffset;
